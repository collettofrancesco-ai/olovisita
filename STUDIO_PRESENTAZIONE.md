...
Tour automatico passo-passo (**34 step totali**, costante `TOTAL`), pensato per mostrare l'app a chi non l'ha mai vista senza dover cliccare manualmente:
- Usa `scheduleDemo(delayMs, callback)` per accodare gli step nel tempo (i delay vanno da 0 a ~283 secondi, calibrati per essere leggibili a voce alta durante una demo dal vivo).
...
- Il tour copre **quattro percorsi in sequenza**:
  1. **Network normale** (step 1-15): programmazione → accettazione Struttura 2 → invio consenso → lettura/scroll/OTP del paziente → verifica OTP → videochiamata → note cliniche → firma referto → consegna al paziente → visita completata.
  2. **Second Opinion Urgente** (step 16-22): richiesta immediata con **consenso presunto** (art. 54 c.p., art. 8 L. 219/2017) → accettazione rapida → videochiamata; in step 21 viene simulata la **notifica sala d'attesa** (toast + campanella + addNotif) per mostrare che il sistema avverte il medico appena il paziente clicca il link Jitsi.
  3. **Accesso Centro Urgente** (step 23-25): il paziente è fisicamente presente, il medico compila il mini-form e lancia subito la videochiamata senza coinvolgere l'altra struttura né attendere la firma del consenso — tutto registrato automaticamente come consenso presunto nella cartella clinica.
  4. **Televisita Centro programmata** (step 26-32): stesso form di prenotazione con "Tipo di Televisita" su Centro → nessuna attesa di accettazione, ma stesso consenso OTP del percorso normale → videochiamata → mai apparsa sul cruscotto dell'altra struttura.
  - Step 33: mostra i **filtri rapidi** (Completate, Annullate, Non Presentato) e la gestione dello storico visite per paziente.
  - Step 34: slide finale di riepilogo.
- Pazienti fittizi: **Maria Esposito** (Network, con note cliniche di esempio `DEMO_NOTES_IT`/`DEMO_NOTES_FR`), **Luca Bianchi** (urgente rete), **Roberto Ferrari** (Centro urgente), **Sara Greco** (Centro programmata).
